...
async def analyze_file_with_brain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        prompt = update.message.caption or "Describe this file in detail."
        response_text = generate_response(api_key=GEMINI_API_KEY, model_name=MODEL_NAME, prompt=prompt, file_path=file_path)
        await update.message.reply_text(response_text)
    except Exception as e:
